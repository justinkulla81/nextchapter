...
// Gate mirrors the candidate-detail page's own visibility check
// (recruiterDatabaseOptIn + not LOCKED/STEALTH) — never trust the caller's
// candidateId alone.
async function requireUnlockedCandidate(candidateId: string) {
  const candidate = await prisma.candidateProfile.findUnique({
    where: { id: candidateId },
    select: { id: true, firstName: true, privacyTier: true, recruiterDatabaseOptIn: true, userId: true },
  })
...
export async function getPoolCandidateResumeSignedUrl(candidateId: string): Promise<string | null> {
  ...
  const candidate = await requireUnlockedCandidate(candidateId)
