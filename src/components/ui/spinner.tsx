...
//
// One fixed CSS @keyframes loop (see nc-flip in globals.css) — not
// randomized per play — but long enough (8.64s) and varied enough (plain
// flips, then one that ends in a clockwise spin, then one that ends in a
// counterclockwise double-spin, then it recycles) that it doesn't read as
// a strict metronome. motion-safe: respects prefers-reduced-motion by not
// flipping at all for users who've asked for reduced motion — the tile
...
export function Spinner({ className, size = 20 }: { className?: string; size?: number }) {
  ...
  return (
    <span
      ...
      aria-label="Loading"
    >
      <span className="relative block h-full w-full [transform-style:preserve-3d] motion-safe:animate-[nc-flip_8.64s_ease-in-out_infinite]">
        <span
