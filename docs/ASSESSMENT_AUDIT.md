...
- **Not done — a genuine open question, not yet raised to the founder**: the spec's dimension→category wiring (§3.2 — Execution feeds Ownership & Reliability + Skills & Execution, Judgment feeds Adaptability, Composure feeds Adaptability + Ownership, Influence feeds Communication & Collaboration + Leadership & Management) is NOT wired into `hireability-grade.ts`. Every existing category score in that file is already a specific weighted blend of 1-2 self-report fields and a reference-trait rating (e.g. `ownershipScore = 0.5 * actionOrientedConfidence + 0.5 * traitFollowThroughRating`, plus structural adjustments). Folding in a 3rd self-report source (and, later, the parallel Reference Check Part A rating) means picking real blend weights that move every candidate's live grade — squarely the kind of judgment call Part 0.1 says to stop and ask about, not guess. Self-vs-reference friction/self-awareness for the performance dimensions (spec's Part 8 data-lineage row "How I Perform → Hiring Manager Notes: self-vs-reference friction, self-awareness label") is unbuilt for the same reason — it would need the same category-blend decision resolved first, or a parallel second self-awareness computation, which is itself a scope call.

**Not yet built** (remaining from Phase 2, plus Phases 5-10 — see spec §11 build order):
