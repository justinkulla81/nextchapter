...
- `src/app/onboarding/actions.ts`'s `updateAssessment` extended with a dormant branch: when the active rotation's `QuadBlock` count is 0 (i.e. a future frozen, quad-block-free rotation), it validates a 1-4 scale and scores via the new Likert-only function instead of the legacy quad+Likert one; otherwise behavior is byte-for-byte unchanged. **`CURRENT_ASSESSMENT_ROTATION_GROUP` has NOT been bumped and no rotation-3 content has been seeded** — this branch is inert in production right now; real candidates still get the live rotationGroup-2 quad+Likert instrument exactly as before.
- **Not yet built**: seeding the frozen bank into the DB under a new rotationGroup + bumping `CURRENT_ASSESSMENT_ROTATION_GROUP`; the BARS-merge read-time averaging (question above); dimension-consumer propagation (`self-awareness.ts`, `dossier-sections.ts`, `coaching-notes.ts` all currently read the old 9-dimension `ASSESSMENT_DIMENSIONS` list for labels/poles — once rotation 3 goes live, these need to become dimension-set-aware, since the old list can't be simply replaced without breaking the archived response's own label rendering); `retake-assessment`/working-style page copy updates for the new dimension names; hard-deleting orphaned rotationGroup-1 content (approved, not yet done); `primaryFunction`'s 3-way consolidation (approved, not yet done).

**Not yet built** (remaining from Phase 2, plus rest of Phase 5, Phases 6-10 — see spec §11 build order):
- Required reference-mix validation/status UI (≥1 manager, ≥1 peer, ≥1 report-or-client) on the candidate-facing references page — actually this was completed in the same commit as Phase 1/2 (`ReferenceCheckSummary`/`computeRequiredMix`); leaving this line for the record.
- Phase 6: Assessments hub page (mostly exists — `/dashboard/skills-assessments` — but "Feeds:" copy per card and the 7-module full roster aren't there yet) + remaining dedup (Skills/Experience, `primaryFunction`) + dead-end fixes (`retake-assessment` back-link, `unlockAction`, `LockedFeatureNotice` stale comment).
